...
- **Single training seed per env** (s221 / s829 / s967). The paired design + noise-seed band control the *noise* variance, **not** training-seed variance. A fully firm claim needs 2–3 training seeds per env (new training runs); until then results are reported as single-training-seed with that caveat explicit. This is the primary threat to external validity. **Mitigation in progress:** `repro/kaggle_notebooks/antmaze_train.ipynb` trains extra AntMaze seeds (252 / 173); once trained, point the eval notebooks' `--resume_ckpt` at them and report the mean over {221, 252, 173} with a training-seed CI.
